		...
		{	
			...
			else
			{
				...
				else
				{
					...
					$sql = 'INSERT INTO users (userLogin, userPass, userEmail, userCode, userImage) VALUES (?, ?, ?, ?, ?)';
					$stmt = $conn->prepare($sql);
					$code = md5($username);
					$stmt->execute([$username, $hash, $email, $code, file_get_contents('../images/default-user.png')]);
